...
> `FIXTURE_VERSION` (from `init_demo.py`) onto the volume after generation,
...
[dependencies]
ailake-core  = "0.1.12"
ailake-query = "0.1.12"   # search(), TableWriter, ContextAssembler, search_multimodal
ailake-store = "0.1.12"   # S3 / GCS / Azure / local backends
```
...
```bash
TAG=v0.1.12          # GitHub release tag (Rust/PyPI version)
JAR_VERSION=0.1.12   # JVM plugin version (gradle, versioned independently — check the release page)
...
│       ├── hnsw.rs             # custom Rust HNSW implementation (no hnsw_rs dep)
