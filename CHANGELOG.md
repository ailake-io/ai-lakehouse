...
- **`compat-bigquery` drops fake-gcs, uses raw multipart HTTP upload**: eliminated `fsouza/fake-gcs-server` and GCS staging entirely. The Python verification step now uploads Parquet files to the BigQuery emulator via a raw `requests` multipart HTTP POST (`uploadType=multipart`) instead of `load_table_from_file()`. Root cause of the hang: `google-cloud-bigquery` always uses `google-resumable-media` for load jobs — the emulator starts a resumable upload session but resets the connection on the chunk PUT (`ConnectionResetError: [Errno 104]`). The `uploadType=multipart` path sends job config + file body in a single request with no chunking, which the emulator handles correctly. Job completion is polled via plain `GET /bigquery/v2/.../jobs/{jobId}`. BQ Python client still used for SQL queries (which work fine). Added `socket.setdefaulttimeout(30)` and step-level `timeout-minutes: 10`.
