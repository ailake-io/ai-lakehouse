...
- **`compat-bigquery` drops fake-gcs, uses `load_table_from_file()`**: eliminated `fsouza/fake-gcs-server` container and all GCS staging entirely; the Python verification step now uploads Parquet files directly from the host filesystem to the BigQuery emulator via `load_table_from_file()` (HTTP POST to the emulator's BQ API endpoint — no GCS path required). Added `socket.setdefaulttimeout(30)` before any network call so a hung HTTP request aborts in ≤30 s instead of blocking indefinitely. Added `timeout-minutes: 10` at the step level as a hard outer deadline. Eliminates the 30-minute hang caused by the emulator's internal GCS client stalling when trying to fetch `gs://` objects via fake-gcs.
