...
- **`airbyte-destination-ailake` package** — Airbyte CDK v3 destination connector that writes Airbyte records to AI-Lake vector tables. Each stream maps to one AI-Lake table at `{table_base_path}/{stream_name}/`. Embedding backends: `cmd` (external process via stdin/stdout JSON protocol), `openai` (OpenAI Embeddings API), `cohere` (Cohere Embed API). Config fields: `table_base_path`, `embed_mode`, `text_field` (dot-notation for nested), `embedding_dim`, `embedding_metric`, `embedding_model` / `embedding_model_version` (propagated to Iceberg properties), `batch_size`, `pre_normalize`, `pq_only`. Airbyte state messages trigger intermediate commits (durability guarantee). Ships with full connector spec JSON, Dockerfile for Airbyte registry, and unit tests.
- **`airbyte-destination-ailake`: `http` embed mode** — `HttpEmbedder` posts to any OpenAI-compatible endpoint using stdlib `urllib` (no extra deps). Request: `{"model": "...", "input": [...]}`. Response: `{"data": [{"embedding": [...]}]}`. Config fields: `http_url` (required), `http_model` (optional, omitted if empty), `http_auth_header` (optional, `Authorization` header value, marked `airbyte_secret`), `http_timeout` (default 60 s). Compatible with Ollama (`/v1/embeddings`), vLLM, LM Studio, Together.ai, Anyscale, Azure OpenAI. Validation requires `http_url` when `embed_mode=http`. Spec JSON and unit tests updated.
- **Airbyte destination demos** — `06_airbyte_destination.ipynb` (Docker demo notebook: config → CmdEmbedder → StreamWriter → simulate `Destination.write()` with state messages → vector search → DuckDB compat → model tracking → migration); `demo/demo_local.py` (standalone script, no Docker/API keys, uses `embed_cmd.py` for deterministic embeddings); `demo/demo_openai.py` (standalone script with real OpenAI embeddings, `--table-path` arg); `demo/embed_cmd.py` (shell-protocol embedder helper). Dockerfile updated to install `airbyte-cdk` + connector package and copy `embed_cmd.py` to `/opt/demo/`.
