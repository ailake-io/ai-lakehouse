...
- **DuckDB `ailake_write_batch` hardcoded `namespace:"default","table":"table"`** (`duckdb-ailake`) — both the 3-arg simple form and the full `AilakeWriteExecFull` wrote to a hardcoded namespace/table regardless of `table_path`. Fixed: table name derived from last path segment of `warehouse` (same logic as `AilakeCatalog`/`AilakeDataSource` in Spark).
