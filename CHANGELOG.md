...
- **`reqwest` workspace dependency now uses `rustls-tls` instead of `default-tls`**: the previous `features = ["json"]` without `default-features = false` enabled `default-tls` → `native-tls` → `openssl-sys`, which failed in the manylinux Docker container (`openssl.pc` not found). Changed to `default-features = false, features = ["json", "rustls-tls"]`; `openssl-sys` no longer appears in the `ailake-py` dependency tree.
