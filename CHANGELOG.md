...
- **`reqwest` removed from `ailake-catalog` default dep tree via `rest-catalog` feature**: `native-tls-vendored` still pulls `openssl-sys` transitively, which fails in manylinux containers even with yum-installed `openssl-devel` because `before-script-linux` runs in a separate shell and pkg-config path changes don't persist into the cargo build environment. Moving `reqwest` (and the `rest`, `databricks` modules) behind the opt-in `rest-catalog` feature eliminates `openssl-sys` from the `ailake-py` dep tree entirely. `cargo tree -p ailake-py -i openssl-sys` now reports "openssl-sys not in tree".
