...
- **Phase 9 propagated to all native plugins** — `partition_by`, `partition_value`, and `partition_filter` added to every SDK and connector:
  - **Spark** (`AilakeNative.search`/`writeBatch`): `partitionBy`/`partitionValue` in `writeBatch()`, `partitionFilter` in `search()` and `searchMultimodal()`; JSON envelope fields `partition_by`, `partition_value`, `partition_filter` (`#[serde(default)]`, backward-compat).
  - **Trino** (`AilakeNative`): same JSON fields; `ailake.partition-by` / `ailake.partition-value` optional catalog properties.
  - **Flink** (`AilakeNativeLoader`): `partitionBy`/`partitionValue` in `writeBatch()`; `partitionFilter` in `search()`; `partition.by` / `partition.value` optional DDL options.
  - **Go** (`ailake-go`): `SearchOptions.PartitionFilter string` (zero-value = no filter); `DataFileEntry.PartitionValue string`; `WriteOptions.PartitionBy string` / `WriteOptions.PartitionValue string`.
  - **C++** (`ailake-cpp`): `SearchOptions.partition_filter std::string` (empty = no filter); `DataFileEntry.partition_value std::string`; `WriteOptions.partition_by` / `WriteOptions.partition_value`.
  - **DuckDB extension**: `partition_filter=` named parameter on `ailake_search` (previously missing from `ailake_search_multimodal` — added); `partition_by=` / `partition_value=` named parameters on `ailake_write_batch`.
  - **Airbyte destination** (`airbyte-destination-ailake`): `partition_by` config field routes each Airbyte stream's records to the correct agent/tenant shard.
  - **Airflow provider**: `AilakeSearchOperator(partition_filter=...)`, `AilakeWriteOperator(partition_by=..., partition_value=...)`, `AilakeHook.search(partition_filter=...)` / `AilakeHook.write_batch(partition_by=..., partition_value=...)`.
- **Tests: `tests/tests/partition_isolation.rs`** — 5 Rust integration tests covering Phase 9: `partition_filter_isolates_per_agent_search` (orthogonal clusters, cosine dist ~0.003 within partition vs ~1.0 across), `partition_filter_nonexistent_returns_empty`, `unfiltered_search_spans_all_partitions`, `score_fn_is_invoked_during_search` (atomic call counter), `score_fn_constant_zero_returns_rows` (all distances = 0.0).

- **Demo fixture Phase 9 (`init_demo.py`)** — added `_write_agent_memory` fixture: two agents (`agent-A`, `agent-B`) write 50 rows each to `/data/ailake_agent` using `partition_by="agent_id"`, demonstrating manifest-level partition pruning and cross-agent isolation. `AGENT_PATH` variable + `DEMO_AGENT_PATH` env var added. `demo_query.json` now includes `table_paths.agent` and `agent` metadata block (`agent_ids`, `partition_column`, `n_docs_per_agent`). `compose-demo.yml` passes `DEMO_AGENT_PATH=/data/ailake_agent` to the notebook container.
- **Demo notebook Phase 9 (`01_ailake_demo.ipynb`)** — added §24–§28 (10 new cells): §24 `ailake.Agent` (remember/recall), §25 partition isolation (`partition_by`/`partition_filter`), §26 `ScoreFn` hybrid scoring, §27 `ToolCallSchema` log+search, §28 `EpisodicMemorySchema` recency decay. ToC updated with §24–28, version bumped to v0.0.20. "Next Steps" links to `08_agents.ipynb`.
- **New notebook `08_agents.ipynb`** — dedicated Phase 9 deep-dive (26 cells): `ailake.Agent`, partition isolation, `ToolCallSchema` logging + semantic search over tool history, `EpisodicMemorySchema` with recency decay curve table, `ScoreFn` hybrid ranking, `assemble_context()` for agent memory, and validation against the pre-generated `init_demo.py` fixture.

### Fixed

- **`TableWriter::create_or_open` part_counter collision** — `create_or_open` previously initialized `part_counter = 0` on every call, causing sequential writers to the same table to produce the same `data/part-00000.parquet` path. The second writer overwrote the first writer's Parquet file. Fix: initialize `part_counter` from `catalog.list_files().len()` so each new writer continues numbering from the last committed file count. Affects any workload calling `create_or_open` on an already-populated table (agents adding shards, multi-process ingest, test fixtures that write two partitions in sequence).
- **DuckDB `ailake_search_multimodal` missing `partition_filter`** — `ailake_search` had a `partition_filter` named parameter but `ailake_search_multimodal` did not. Added `partition_filter=` to `ailake_search_multimodal` bind data, registration, and `AilakeLib::search_multimodal` signature. Propagated through `AilakeMultimodalBindData.partition_filter`, `AilakeMultimodalBind` named-param parse, and JSON envelope injection (only included when non-empty, backward-compat).
- **Python SDK `partition_filter` missing from `search()` and `search_multimodal()`** — `ailake-py/src/lib.rs`: `search()` hardcoded `partition_filter: None` in `SearchConfig`; `search_with_data()` PyO3 parameter was named `partition_value` (inconsistent with Rust `partition_filter`); `search_multimodal()` also hardcoded `None`. All three now accept `partition_filter: Option<String>` and pass it through to `SearchConfig`. `SearchQuery`, `search()` module function, and `_ailake.pyi` stubs updated to match.
- **Python SDK `score_fn` not wired in `SearchQuery`** — `SearchQuery._execute_arrow()` and `search()` accepted no `score_fn` parameter. Added `score_fn: Callable | None` to `SearchQuery.__init__()` and `search()` module function. Python-level `_apply_score_fn()` helper re-ranks the Arrow Table by calling `score_fn(distance: float, row: dict)` per row (dict maps column name → scalar value, lower return = ranked first).
- **GPU flat-scan (`SearchSession.search_batch`) does not apply `score_fn`** — documented in `docs/specs/GPU_FFI_EVALUATION.md §9` and `SETUP.md §8F-7`. Architectural: `SearchSession` has no Parquet row data during the deferred build window; score_fn cannot be applied. Workaround: use synchronous `write_batch()` or wait for `IndexStatus::Ready`. Python-level `score_fn` (post-processing on IPC bytes) is unaffected.
