...
- **Python full-read after search** — `ailake.search(..., fetch_data=True)` and `Table.search(..., fetch_data=True)` return a `SearchQuery` whose `.to_arrow()` / `.to_pandas()` / `.to_polars()` / async variants materialise a full `pyarrow.Table` with all Parquet columns + `_distance: float32` instead of pointer-only dicts. Backward-compatible: default `fetch_data=False` behaviour unchanged. Implemented via `ailake_query::fetch_rows` (groups results by file, reads Parquet once per file via `arrow_select::take`) + new `search_with_data` PyO3 function (serialises `RecordBatch` to Arrow IPC bytes via `arrow-ipc`; Python side deserialises with `pyarrow.ipc.open_file`).
