...
- **`compat-bigquery` Python verification step fixed**: (1) `python3 -u -` forces unbuffered stdout so logs appear immediately; (2) `BIGQUERY_EMULATOR_HOST` set to `host:port` format (no `http://` scheme — the client adds it) and set before importing the BigQuery library; (3) explicit `ClientOptions(api_endpoint=...)` passed to `bigquery.Client` as belt-and-suspenders; (4) switched from external tables to `load_table_from_uri` (more reliably supported by `goccy/bigquery-emulator`); (5) all API calls have explicit `timeout=` parameters; (6) wait loop now fails loudly with `exit 1` if a service never becomes ready; (7) BQ emulator health check uses TCP-connect-only curl (accepts any HTTP status, not `-f`) since `/` returns non-200 on a fresh emulator.
- **`compat-bigquery` uses random host ports**: fixed "address already in use" on port 9050 by switching both fake-gcs-server and bigquery-emulator to `127.0.0.1::PORT` (Docker-assigned random host port); actual ports captured via `docker port` and exported as `FAKE_GCS_PORT`/`BQ_EMULATOR_PORT` env vars for subsequent steps and the Python verification script. Eliminates conflicts between concurrent workflow runs and system services on self-hosted runners.
