...
# Output:
ls build/libs/trino-plugin-0.1.4-plugin.jar
```
...
ls build/libs/spark-plugin-0.1.4-plugin.jar
```
...
```bash
spark-submit \
  --jars spark-plugin-0.1.4-plugin.jar \
  --conf spark.sql.extensions=io.ailake.spark.AilakeSparkExtensions \
...
ls build/libs/ailake-flink-0.1.4-plugin.jar
```
...
```sql
-- Flink SQL Client
ADD JAR '/path/to/ailake-flink-0.1.4-plugin.jar';
...
spark-shell --jars /path/to/spark-plugin-0.1.4-plugin.jar \
  --conf spark.sql.extensions=io.ailake.spark.AilakeSparkExtensions
```
