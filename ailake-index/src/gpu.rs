...
//!
//! Two independent GPU backends:
//!   - NVIDIA CUDA: candle-core matmul (compile-time `gpu` feature, runtime CUDA probe).
//!   - AMD ROCm:    hipBLAS SGEMM via libloading (no compile-time dependency, runtime HIP probe).
//!
//! Callers receive `None` when neither GPU is available; fall back to CPU.
