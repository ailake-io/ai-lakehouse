...
- When ready for release, maintainers bump versions, update CHANGELOG, merge to `main`, run CI and Compat Heavy, then trigger `release.yml` — a single workflow that publishes crates, JVM plugins, Airflow provider, and Python wheels in a sequential chain (see [`docs/contributing/TESTING.md`](./docs/contributing/TESTING.md#manual-actions-trigger-order-pre-release)).
