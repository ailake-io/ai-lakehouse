...
`JdbcCatalog`/`GlueCatalog`/`RestCatalog` (below) reach the same observable `Append`/`Delete`/`Replace`/`Overwrite` semantics as this table. All three now share `HadoopCatalog`'s real Avro-manifest logic via `ailake-catalog/src/manifest_commit.rs` (`build_commit`/`apply_commit`/`commit_into_metadata`) instead of the flat single-JSON-file-per-snapshot format they used before — same V3 `first_row_id` assignment, Phase F Puffin stats, and Phase J partition-stats support Hadoop has. `commit_snapshot` still explicitly rebuilds the effective file/equality-delete list from the previous snapshot on every `Append`/`Delete` (fetched fresh on each OCC retry attempt, so a concurrent writer's prior commit is never lost) — for Jdbc/Glue this happens inside their own OCC retry loop before calling `commit_into_metadata`; for Rest it's handled by `build_commit` itself reading the manifest-list chain, same as Hadoop. `list_files()` on Jdbc/Glue now returns **absolute** paths (Iceberg spec requirement), matching Hadoop's existing behavior — see ADR-020 in `docs/contributing/DECISIONS.md`.
