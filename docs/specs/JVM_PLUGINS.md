...
```bash
TAG=v0.1.5          # GitHub release tag — replace with desired release (Rust/PyPI version)
...
# Output
ls -lh build/libs/trino-plugin-0.1.5-plugin.jar
```
...
ls -lh build/libs/spark-plugin-0.1.5-plugin.jar
```
...
```bash
PLUGIN_JAR=$(pwd)/spark-plugin/build/libs/spark-plugin-0.1.5-plugin.jar
