...
```bash
TAG=v0.1.9          # GitHub release tag — replace with desired release (Rust/PyPI version)
JAR_VERSION=0.1.9   # JVM plugin version — gradle, versioned independently of TAG; check the release page
...
# Output
ls -lh build/libs/trino-plugin-0.1.9-plugin.jar
```
...
cp build/libs/trino-plugin-0.1.9-plugin.jar $TRINO_HOME/plugin/ailake/
...
ls -lh build/libs/spark-plugin-0.1.9-plugin.jar
```
...
```bash
PLUGIN_JAR=$(pwd)/spark-plugin/build/libs/spark-plugin-0.1.9-plugin.jar
...
$SPARK_HOME/bin/spark-shell \
  --jars $(pwd)/spark-plugin/build/libs/spark-plugin-0.1.9-plugin.jar \
  --conf spark.sql.extensions=io.ailake.spark.AilakeSparkExtensions \
