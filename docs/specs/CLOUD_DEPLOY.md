...
# → spark-plugin/build/libs/spark-plugin-0.1.9-plugin.jar

# Trino plugin fat-jar
cd trino-plugin && ./gradlew shadowJar
# → trino-plugin/build/libs/trino-plugin-0.1.9-plugin.jar
...
aws s3 cp spark-plugin/build/libs/spark-plugin-0.1.9-plugin.jar \
  ...
  --configurations '[
    {
      ...
      "Properties": {
        "spark.jars":                          "s3://my-bucket/ailake/jars/spark-plugin-0.1.9-plugin.jar",
...
NumberOfWorkers: 10
DefaultArguments:
  --extra-jars: "s3://my-bucket/ailake/jars/spark-plugin-0.1.9-plugin.jar"
  --additional-python-modules: "ailake==0.1.9"
...
sudo aws s3 cp s3://my-bucket/ailake/jars/trino-plugin-0.1.9-plugin.jar \
...
gsutil cp spark-plugin/build/libs/spark-plugin-0.1.9-plugin.jar \
...
gcloud dataproc clusters create ailake-cluster \
  ...
  --properties \
    spark:spark.jars=gs://my-bucket/ailake/jars/spark-plugin-0.1.9-plugin.jar,\
...
databricks fs cp spark-plugin/build/libs/spark-plugin-0.1.9-plugin.jar \
    dbfs:/FileStore/ailake/jars/spark-plugin-0.1.9-plugin.jar
```
...
```json
{
  ...
  "libraries": [
    { "jar": "dbfs:/FileStore/ailake/jars/spark-plugin-0.1.9-plugin.jar" },
    { "pypi": { "package": "ailake==0.1.9" } }
  ],
  "init_scripts": [
...
spark-submit \
  --master yarn \
  --jars wasbs://ailake@mystorageaccount.blob.core.windows.net/jars/spark-plugin-0.1.9-plugin.jar \
  --conf spark.sql.extensions=io.ailake.spark.AilakeSparkExtensions \
...
spark-submit \
  ...
  --conf "spark.driver.extraJavaOptions=-Djava.library.path=/opt/ailake/lib" \
  --conf "spark.executor.extraJavaOptions=-Djava.library.path=/opt/ailake/lib" \
  --jars s3a://my-bucket/jars/spark-plugin-0.1.9-plugin.jar \
...
cd ailake-flink
./gradlew shadowJar
# → build/libs/ailake-flink-0.1.9-plugin.jar
```
...
4. Upload `ailake-flink-0.1.9-plugin.jar` as an application JAR.
...
flink run \
  ...
  -c my.job.Main ailake-flink-0.1.9-plugin.jar
```
...
| Flink `ClassNotFoundException: io.ailake.flink.*` | Fat JAR not on classpath | Copy `ailake-flink-0.1.9-plugin.jar` to `$FLINK_HOME/lib/` or use `flink run -C` |
