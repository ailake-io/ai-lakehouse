...
Two behaviors assumed at design time turned out to be wrong when verified against a **live** `ducklake` extension (not just its documentation), both discovered while writing the integration test for this backend:
...
- Not wired into `ailake-cli`/`ailake-py`'s catalog selection surface. Confirmed this is consistent with, not behind, the rest of the project: `JdbcCatalog`/`GlueCatalog`/`NessieCatalog` aren't wired in either (`grep -rn "JdbcCatalog" ailake-cli ailake-py` — no hits outside `ailake-catalog` itself and one integration test). That wiring is a distinct, not-yet-started piece of work for every alternative backend.
