...
**Decision**: Rather than hand-writing the same ~320-line Avro-manifest read/write logic three more times (Glue, Jdbc, and matching `HadoopCatalog`'s own copy), extract it once into `ailake-catalog/src/manifest_commit.rs`: `commit_into_metadata(store, table_root, warehouse, meta, snapshot)` (mutates an `IcebergMetadata` in place — manifest write, V3 first_row_id, Puffin, partition-stats, schema/property merge — without persisting it), `list_files_from_metadata`/`list_equality_deletes_from_metadata` (read side). Every backend that owns its own `metadata.json` (Hadoop, Glue, Jdbc) calls these; each backend supplies only its own "read old state, write new pointer with OCC/CAS" glue around the shared core (`HadoopCatalog::save_metadata`, Glue's version-id-guarded `update_table`, Jdbc's conditional `UPDATE` on `metadata_location`). `HadoopCatalog` itself was refactored to call the extracted functions instead of keeping a parallel copy, verified against its own pre-existing 6-test suite (including the V3 first_row_id monotonicity and Replace-doesn't-inherit-previous-manifest regression tests) with zero behavior change.

**Consequences**:
- Glue and Jdbc gained full V2/V3 manifest parity with Hadoop (Avro manifests, first_row_id, Puffin stats, partition stats) in one pass, verified two different ways given the two backends' different testability: Jdbc got new real-SQLite end-to-end coverage (`evolve_schema` + equality-delete accumulation added to its existing test); Glue has no live-AWS test in this codebase (true before this change too — nothing here tests Glue against real AWS), so its correctness rests on sharing the exact code path Hadoop's tests already exercise, not on independent verification.
...
- A future bug fix or feature in the shared manifest logic (e.g. a V3 spec compliance fix) now lands for Hadoop, Glue, and Jdbc simultaneously instead of needing to be ported three times and risking silent drift between backends — the original problem this ADR responds to.
- `RestCatalog`/`NessieCatalog` were deliberately not migrated to the shared module — they don't own a `metadata.json` file the way Hadoop/Glue/Jdbc do (a REST server does), so `commit_into_metadata`'s "mutate an owned `IcebergMetadata`, caller persists it" contract doesn't fit without first teaching the module to drive REST protocol updates instead of writing files via `Store` directly. Left as a documented follow-up (see `CHANGELOG.md`), not attempted in this pass.

**Rejected alternatives**:
- Hand-roll matching Avro-manifest logic separately in `glue.rs`/`jdbc.rs`: rejected — the whole reason this gap existed for years is that these two backends drifted from Hadoop's Phase 2 manifest work in the first place; writing separate copies risks the exact same drift recurring on the next feature.
