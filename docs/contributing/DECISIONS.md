...
1. **FFI is not the bottleneck relative to S3, but the JSON cost was undercounted.** Write throughput is dominated by S3 PUT (~4000ms). The original ~10ms estimate here only measured Rust-side `serde_json` decode; it missed the JVM-side cost of *producing* that JSON in the first place. Measured directly (JDK 21, 1k×1536 `Float.toString`-based `mkString` + UTF-8 encode, the same construction `AilakeNative.scala` performs): **~150ms/batch** (134ms string-build + 12ms UTF-8 encode), not ~10ms — an order of magnitude higher, ~3.75% of the S3 baseline rather than 0.25%. Still not dominant for large batches, but real under sustained/concurrent writes (temporary ~16MB `String` + resizing `StringBuilder` per call — GC pressure that a flat ms figure doesn't capture) and proportionally worse for smaller/more frequent batches. Search JSON is still ~6KB (one query vector) — parse time there remains noise vs HNSW traversal and S3 I/O; this correction applies to the write path only.
...
**Next incremental step (Fase 10)**: Replace the JSON embedding payload in `ailake_write_batch_json` with Arrow IPC bytes passed via JNI, via a new `ailake_write_batch_ipc` entry point (JSON path kept as-is for backward compatibility). Eliminates the real ~150ms/1k-vecs JVM-side cost (not the originally-estimated ~10ms) without adding a daemon or changing the calling convention. **Scope correction**: only `ailake-jni`'s three JVM consumers (Spark, Trino, Flink) pay this cost at all — PyO3 is already zero-copy in-process (point 3), and the CLI/Go/C++ paths never built this JSON payload to begin with. Of the three, only Spark currently has Arrow on its build classpath (`spark-plugin/build.gradle.kts` `compileOnly`s Spark itself, which bundles `arrow-vector` transitively); Trino's and Flink's plugin builds declare no Arrow dependency at all today, so wiring IPC there means adding a new dependency to each, with the same classpath-weight/version-pinning risk flagged in point 6 (smaller in degree — `arrow-vector` alone is ~5-10MB, not the 50MB+ of the full Flight client — but the same class of risk). **Implemented for Spark only** (`ailake_write_batch_ipc`, `ailake-jni/src/lib.rs`); Trino and Flink deliberately deferred pending a real measured write-volume case for adding the dependency to those two plugins — see the TODO in Fase 10 of `CLAUDE.md`.
...
- **Remaining gap closed (`ailake-cli/src/serve.rs`)**: everything above still required an operator (or a cron) to actually invoke `ailake compact` — a long-running `ailake serve` process saw the same foreign file degrade every `/search` call indefinitely otherwise. `handle_search` now calls `maybe_probe_auto_compact` after every search: rate-limited to at most one extra `list_files` call per `AUTO_COMPACT_CHECK_COOLDOWN_MS` (60s, server-wide via a CAS'd timestamp, not per-request), it checks `DataFileEntry::is_foreign()` and, if any file qualifies, spawns one background `CompactionExecutor::run()` pass (`auto_compact_inflight` prevents a second probe from starting an overlapping pass). Search latency is unaffected — the probe is fire-and-forget. `/compact` remains manual and untouched; this only removes the need to schedule it externally. Covered by real (non-mocked) `HadoopCatalog` + `TableWriter` round-trip tests in `serve.rs`'s `tests` module.
...
## ADR-020: Extract HadoopCatalog's manifest logic into a shared module instead of re-implementing it per backend

**Date**: 2026-07
**Status**: Accepted

**Context**: A capability-parity audit across all 6 `CatalogProvider` backends found that `RestCatalog`, `GlueCatalog`, and `JdbcCatalog` had never adopted the real Iceberg Avro-manifest format `HadoopCatalog` uses — they still wrote a single flat JSON file per snapshot (`crate::snapshot::Manifest`), a Phase 1 stopgap `snapshot.rs`'s own header comment already flagged as superseded ("Phase 2: replace with apache-avro for Iceberg spec compliance") back when `HadoopCatalog` got real manifests. Three concrete consequences: no V3 `first_row_id` row-lineage assignment, no Phase F Puffin stats (vector centroids + BM25 blooms), no Phase J partition-stats Parquet — and, found first and worse, `commit_snapshot` on all three silently discarded `snapshot.equality_delete_files` entirely, so `delete_rows` against those backends did nothing detectable.

**Decision**: Rather than hand-writing the same ~320-line Avro-manifest read/write logic three more times (Glue, Jdbc, and matching `HadoopCatalog`'s own copy), extract it once into `ailake-catalog/src/manifest_commit.rs`, split into two layers specifically so a metadata-owning backend (Hadoop/Glue/Jdbc) *and* a metadata-delegating backend (Rest, whose actual state lives on a server, not in a file we own) can both reuse the real Avro-manifest-writing logic:
- `build_commit(store, table_root, warehouse, meta, snapshot)` does all the actual work — writes the Avro data/delete manifest(s), chains the manifest list, assigns V3 `first_row_id`s, writes Puffin/partition-stats files — and returns a `CommitArtifacts` bundle. It only *reads* `meta` (current schema/snapshots/next-row-id); it never mutates it.
- `apply_commit(meta, artifacts)` folds a `CommitArtifacts` bundle into an owned `IcebergMetadata` in place (push snapshot, patch schema, merge properties/stats refs) — what Hadoop/Glue/Jdbc persist as their next `metadata.json`. `commit_into_metadata()` is the `build_commit`+`apply_commit` convenience wrapper those three actually call, each supplying only its own "read old state, write new pointer with OCC/CAS" glue around the shared core (`HadoopCatalog::save_metadata`, Glue's version-id-guarded `update_table`, Jdbc's conditional `UPDATE` on `metadata_location`).
- `RestCatalog::commit_snapshot` calls `build_commit` directly and translates the same `CommitArtifacts` into REST protocol `TableUpdate`s (`add-snapshot`, `add-schema`/`set-current-schema` for Phase G/I schema changes, and two new update variants — `set-statistics`/`set-partition-statistics` — for Phase F/J stats refs, mapped field-for-field onto the REST spec's own `StatisticsFile`/`PartitionStatisticsFile` schemas). `NessieCatalog` inherits this for free via its existing pure delegation to `RestCatalog`.

`HadoopCatalog` itself was refactored to call the extracted functions instead of keeping a parallel copy, verified against its own pre-existing 6-test suite (including the V3 first_row_id monotonicity and Replace-doesn't-inherit-previous-manifest regression tests) with zero behavior change.

**Consequences**:
- All 6 `CatalogProvider` backends now share one Avro-manifest implementation instead of three (Hadoop/Glue/Jdbc via `apply_commit`, Rest/Nessie via direct `TableUpdate` translation) — a future bug fix or V3 spec-compliance change lands everywhere at once instead of needing to be ported N times, the original problem this ADR responds to.
- Glue and Jdbc gained full V2/V3 manifest parity with Hadoop (Avro manifests, first_row_id, Puffin stats, partition stats) in one pass, verified two different ways given the two backends' different testability: Jdbc got new real-SQLite end-to-end coverage (`evolve_schema` + equality-delete accumulation added to its existing test); Glue has no live-AWS test in this codebase (true before this change too), so its correctness rests on sharing the exact code path Hadoop's tests already exercise, not on independent verification.
- `DataFileEntry::path` returned by `list_files` on Glue/Jdbc is now **absolute** (Iceberg spec requirement for manifest file paths — matches Hadoop's existing behavior) rather than warehouse-relative, since it now goes through the real Avro manifest writer's path-resolution logic instead of a flat JSON dump of whatever was passed in. Any caller asserting an exact relative-path string against Glue/Jdbc's `list_files` output needs updating to `ends_with()` or equivalent (the JDBC catalog's own test needed exactly this fix, caught immediately by the existing test suite).
- The REST protocol has no update to correct an existing partition spec's `source-id` in place (needed for the rare Phase I remap case, when a partition column's field-id shifts once the bootstrap schema is replaced by the real one). Handled via `add-spec` + `set-default-spec` instead — spec-correct (confirmed against `apache/iceberg`'s `rest-catalog-open-api.yaml`), registers the corrected spec as new and makes it default rather than mutating the old one.
- `next-row-id` needs no explicit REST update at all — the spec's own field description states the server derives it from `first_row_id`/`record_count` already present in the manifest, which `build_commit` assigns correctly regardless of caller. Confirmed by reading the spec rather than assuming REST needed the same client-side counter-tracking Hadoop/Glue/Jdbc do.
- No live REST server available in this environment to test `RestCatalog`'s new `CommitArtifacts` → `TableUpdate` translation against — verified via compilation, the existing 54-test unit suite (URL/serialization shape checks), and `build_commit` itself already being proven correct through Hadoop/Glue/Jdbc's real tests. The translation layer itself (which `TableUpdate`s get sent, in what shape) is new and unverified against a real server's actual acceptance/rejection behavior.
