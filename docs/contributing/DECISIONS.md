...
- The CLI (`ailake compact`) and `ailake serve`'s `/compact` endpoint already built the correct (full) file list independently of `CompactionExecutor::run()` — they were unaffected by the Replace bug, but benefit from the same foreign-file prioritization in `CompactionPlanner::plan()`.
