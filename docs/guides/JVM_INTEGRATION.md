...
```bash
TAG=v0.1.4          # GitHub release tag — replace with target release (Rust/PyPI version)
...
```bash
PLUGIN_JAR=/opt/ailake/spark-plugin-0.1.4-plugin.jar
...
databricks fs cp spark-plugin-0.1.4-plugin.jar \
...
cp trino-plugin-0.1.4-plugin.jar $TRINO_HOME/plugin/ailake/
...
flink run \
  --jar my-pipeline.jar \
  --classpath ailake-flink-0.1.4-plugin.jar \
...
```bash
cp ailake-flink-0.1.4-plugin.jar /opt/flink/lib/
cp libailake_jni.so               /opt/ailake/lib/
