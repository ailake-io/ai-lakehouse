...
```bash
TAG=v0.1.5          # GitHub release tag — replace with target release (Rust/PyPI version)
JAR_VERSION=0.1.5   # JVM plugin version — gradle, versioned independently of TAG; check the release page
...
```bash
PLUGIN_JAR=/opt/ailake/spark-plugin-0.1.5-plugin.jar
...
databricks fs cp spark-plugin-0.1.5-plugin.jar \
...
cp trino-plugin-0.1.5-plugin.jar $TRINO_HOME/plugin/ailake/
...
flink run \
  --jar my-pipeline.jar \
  --classpath ailake-flink-0.1.5-plugin.jar \
...
```bash
cp ailake-flink-0.1.5-plugin.jar /opt/flink/lib/
cp libailake_jni.so               /opt/ailake/lib/
