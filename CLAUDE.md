...
- [x] **RaBitQ (Random Binary Quantization)** — índice flat com 1 bit/dim (16× menor que F16; 200 bytes/vetor para dim=1536 vs 3072 bytes F16). Melhor recall que binary quantization ingênuo via **rotação ortogonal modificada Gram-Schmidt** (P^T·P = I; preserva produtos internos exatamente) + estimador IP não-viesado (XOR + popcount). Throughput ~163k vec/s medido em SIFT-1M (sem k-means, sem grafo). Busca `search(&self)` — scan sequential interno O(N) com `select_nth_unstable_by` O(N) para seleção de candidatos; paralelismo por shard no nível externo via rayon. `RaBitQCodebook::estimate_ip_binary(b_q, q_scale, entry)` aceita query pré-binarizada — binarizar uma vez antes do loop elimina `bits_from_signs` repetidos. Benchmark SIFT-1M: QPS=101 (Euclidean), recall@10=0.67 (rerank=3); recomendado para workloads **cosine** (recall 0.85–0.95 com rerank≥3 em embeddings reais). Reranking com F16 brutos opcional (`keep_raw=true`, recomendado). Schema: `ailake_core::schema::RaBitQConfig { seed: u64, keep_raw: bool }` re-exportado do crate root. Módulos: `ailake_vec::rabitq` (RaBitQCodebook, RaBitQVec, encode_batch, bits_from_signs), `ailake_index::rabitq` (RaBitQIndex, RaBitQSerializer). Flag de footer: `FLAG_INDEX_RABITQ = 0x0002` (bit 1). `AilakeFileWriter::new` auto-seleciona RaBitQ quando `policy.rabitq.is_some()` — não requer chamada explícita a `with_index_type`. Exposição: CLI `ailake create --rabitq [--rabitq-seed N] [--rabitq-keep-raw]`, Python `TableWriter(rabitq=True, rabitq_seed=0, rabitq_keep_raw=True)`, JNI JSON `"rabitq":true,"rabitq_seed":0,"rabitq_keep_raw":true`. Go SDK: `FlagIndexRaBitQ=0x0002`, `IsRaBitQ()`, `DeserializeRaBitQ`, `RaBitQIndex.Search` com reranking F16; `chacha12.go` implementa ChaCha12 PRNG + splitmix64 seed expansion idêntico ao Rust `StdRng::seed_from_u64` — matrizes de projeção bit-idênticas entre Rust e Go para mesmo seed. C++ SDK: `kFlagIndexRaBitQ=0x0002`, `AilakeHeader::is_rabitq()`, `deserialize_rabitq`, `rabitq_search`; `chacha12.hpp` com `ChaCha12Rng` + `splitmix64_expand` — mesmo comportamento. Use com `rerank_factor ≥ 3` (cosine) ou `≥ 10` (Euclidean/datasets complexos). Teste de integração: `rabitq_write_search_returns_correct_top_result`.
- [x] **Binary quantization Hamming SIMD** — `BinaryIndex` flat index: 1 bit/dim MSB-first (`bit_i = x_i >= 0.0`), sem rotação. Storage: 192 bytes/vetor dim=1536 (32× menor que F32). Kernels SIMD: AVX2+SSSE3 nibble-LUT+PSADBW (32 bytes/iter) → NEON `vcntq_u8` (16 bytes/iter) → scalar u64 `count_ones`. Throughput >200k vec/s. Schema: `ailake_core::schema::BinaryConfig { keep_raw: bool }`. Módulos: `ailake_vec::binary_quant` (`binary_quantize`, `hamming_distance`), `ailake_index::binary` (`BinaryIndex`, `BinarySerializer`). Flag: `FLAG_INDEX_BINARY = 0x0004` (bit 2). Auto-selecionado quando `policy.binary.is_some()` (verificado antes de rabitq). Exposição: CLI `ailake create --binary [--no-binary-keep-raw]`, Python `TableWriter(binary=True, binary_keep_raw=True)`, JNI JSON `"binary":true,"binary_keep_raw":true`. Go SDK: `FlagIndexBinary=0x0004`, `IsBinary()`, `DeserializeBinary`, `(BinaryIndex).Search` com reranking F16; `binary.go`. C++ SDK: `kFlagIndexBinary=0x0004`, `is_binary()`, `deserialize_binary`, `binary_search`; `binary.hpp` com SIMD dispatch. 14 testes C++ em `tests/test_binary.cpp`. Recall@10 cosine: 0.50–0.70 (sem raw); 0.80–0.92 (com reranking F16, rerank_factor≥3).
...
├── ailake-index/         # HNSW via hnsw_rs, IVF-PQ, RaBitQ, Binary Hamming — serialização bincode, mmap
