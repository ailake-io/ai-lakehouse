...
O formato é um **único arquivo Parquet estendido** com seções adicionais após o footer Parquet padrão:
...
│  - file metadata: ailake.hnsw_offset, ailake.hnsw_len           │
│  - 4 bytes: "PAR1"                                              │
├─────────────────────────────────────────────────────────────────┤
│  ▼ AILAKE FOOTER EXTENSION (após PAR1 final)                    │
│                                                                 │
│  RODAPÉ AI-LAKE (ÍNDICE VELOCE):                                │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  AILAKE MAGIC (4 bytes: "AILK")                         │    │
│  │  AILAKE HEADER (versão, ofssets, métricas)              │    │
│  ├─────────────────────────────────────────────────────────┤    │
│  │  GRAFO HNSW SERIALIZADO (bincode)                       │    │
│  │  - Camadas hierárquicas                                 │    │
│  │  - Conexões entre nós                                   │    │
│  │  - Vetores indexados (referenciados por RowId)          │    │
│  ├─────────────────────────────────────────────────────────┤    │
│  │  ESTATÍSTICAS PARA PRUNING                              │    │
│  │  - Centróide Geométrico (vetor médio do arquivo)        │    │
│  │  - Raio Máximo (maior distância ao centróide)           │    │
│  │  - Métricas suportadas (Cosine, L2, Inner Product)      │    │
│  ├─────────────────────────────────────────────────────────┤    │
│  │  AILAKE FOOTER (8 bytes: offset + length do rodapé)     │    │
│  │  AILAKE MAGIC END (4 bytes: "AILK")                     │    │
│  └─────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
```
...
Leitores Parquet padrão (PyIceberg, Spark, Trino, DuckDB) param de ler ao encontrar o segundo `PAR1` (final do footer Parquet). A extensão AI-Lake após esse marcador é **invisível e inofensiva**:
...
File: data/part-00001.parquet
  ...
  ailake.hnsw-offset: 12582912                    (byte offset do rodapé HNSW)
  ailake.hnsw-len: 4194304                        (byte length do rodapé HNSW)
```
...
2. **Lê do footer Parquet**: extrai `ailake.hnsw_offset` e `ailake.hnsw_len` do key-value metadata.
3. **Segundo `GET`**: baixa apenas os bytes do rodapé AI-Lake (`Range: bytes=hnsw_offset-(hnsw_offset+hnsw_len)`).
4. **Localmente**: salva os bytes em arquivo temporário e abre via `memmap2::Mmap`.
5. **Deserializa via `bincode`**: o grafo HNSW é carregado preguiçosamente — apenas as páginas tocadas durante a busca são paginadas pelo SO.
...
let hnsw_offset = parquet_meta.kv_metadata("ailake.hnsw_offset")?.parse::<u64>()?;
let hnsw_len = parquet_meta.kv_metadata("ailake.hnsw_len")?.parse::<u64>()?;

let hnsw_bytes = store.get_range(path, hnsw_offset..hnsw_offset + hnsw_len).await?;
...
- [ ] Documentação de spec pública do formato
- [ ] Reranking automático após PQ
