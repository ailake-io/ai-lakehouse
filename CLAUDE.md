...
- [x] **RaBitQ (Random Binary Quantization)** — índice flat com 1 bit/dim (16× menor que F16; 200 bytes/vetor para dim=1536 vs 3072 bytes F16). Melhor recall que binary quantization ingênuo via rotação aleatória + estimador IP não-viesado (XOR + popcount). Throughput ~300k vec/s (sem k-means, sem grafo). Reranking com F16 brutos opcional (`keep_raw=true`, recomendado). Schema: `ailake_core::schema::RaBitQConfig { seed: u64, keep_raw: bool }` re-exportado do crate root. Módulos: `ailake_vec::rabitq` (RaBitQCodebook, RaBitQVec, encode_batch), `ailake_index::rabitq` (RaBitQIndex, RaBitQSerializer). Flag de footer: `FLAG_INDEX_RABITQ = 0x0002` (bit 1). `AilakeFileWriter::new` auto-seleciona RaBitQ quando `policy.rabitq.is_some()` — não requer chamada explícita a `with_index_type`. Exposição: CLI `ailake create --rabitq [--rabitq-seed N] [--rabitq-keep-raw]`, Python `TableWriter(rabitq=True, rabitq_seed=0, rabitq_keep_raw=True)`, JNI JSON `"rabitq":true,"rabitq_seed":0,"rabitq_keep_raw":true`. Go SDK: `FlagIndexRaBitQ=0x0002`, `IsRaBitQ()`, `DeserializeRaBitQ`, `RaBitQIndex.Search` com reranking F16. Use com `rerank_factor ≥ 3` para melhor recall. Teste de integração: `rabitq_write_search_returns_correct_top_result`.
