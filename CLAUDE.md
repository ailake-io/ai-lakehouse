...
### Fase 21 — Trino/Spark `ef_search`/`pruning_threshold` + 6 novos C-ABI ops em `ailake-jni` (Spark/Trino/Flink)

> **Contexto (2026-07-28)**: pedido do usuário pra auditar trino-plugin de novo achou 2 gaps reais salvos em memória pra implementar depois — "salve isso para implementar daqui a pouco" — e depois "pode implementar os dois gaps, crie uma branch nova pra isso" (branch `feature/jvm-search-tuning-and-jni-ops`, off `develop` pós-merge do PR #108).

- [x] **`search()` não repassava `ef_search`/`pruning_threshold` (Trino + Spark)** — `ailake_search_json`'s `Req` struct (`ailake-jni/src/lib.rs`) sempre aceitou os dois campos, mas nenhum dos dois plugins mandava nenhum (Flink já mandava, via `search.ef` na DDL). Trino: `VectorScanSplit` ganhou `efSearch: Int = 0`/`pruningThreshold: Float = -1f` (sentinels não-nulos, mesmo padrão de `topK`/`hybridWeight` — evita introduzir campo nullable na classe JSON-serializada entre coordinator/worker, área com histórico documentado de bug real de serialização Kotlin/Jackson), 2 novas session properties (`ailake.ef_search`/`ailake.pruning_threshold`), convertidos pra `null` (sentinel→omitido do request) só no ponto de chamada de `AilakeNative.search()`. Spark: mesmos 2 parâmetros `Option[Int]`/`Option[Float]` em `AilakeNative.search`/`implicits.ailakeSearch` (sem risco de serialização, é Scala puro).
...
- Docs atualizados: `docs/guides/JVM_INTEGRATION.md` §7 (nova seção "decay_memories / migrate / delete_rows / add_vector_column / backfill_vector_column / estimate") com tabela cross-engine + exemplos SQL/Scala pros 3 motores.
