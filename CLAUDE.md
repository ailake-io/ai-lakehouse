...
- [ ] **`ailake_scan_json` — search + fetch de linhas completas, sem wrapper em nenhum plugin JVM.** Hoje só é consumida pela extensão DuckDB (`duckdb-ailake`) — devolve `{"ok":true,"schema":[...],"num_rows":N,"columns":{"id":[...],"text":[...],"_distance":[...]}}`, ou seja, as colunas reais da tabela (não só `row_id`/`distance`/`file_path`). Sem ela, toda busca via SQL nos três plugins retorna apenas o resultado geométrico e exige um `JOIN` manual contra uma tabela Iceberg registrada separadamente para recuperar `chunk_text`/`document_title`/etc. (ver `docs/guides/JVM_INTEGRATION.md` §4D) — round-trip extra e outra conexão de catálogo a manter sincronizada. Plano:
  - **Trino**: nova tabela `ailake.default.search_full` com colunas dinâmicas — schema resolvido uma vez na inicialização do connector a partir de uma property de catálogo `ailake.fetch-columns` (mesmo padrão de `ailake.text-columns` no `ingest`, já que o schema de um catalog Trino é fixo por processo).
  - **Spark**: `spark.ailakeSearchWithData(tableUri, queryVector, topK, columns: Seq[String])` retornando `DataFrame` com schema dinâmico construído a partir de `columns`; evitar duplo round-trip JSON reaproveitando o payload Arrow IPC da Fase 10 se `ailake_scan_json` ganhar uma variante IPC.
  - **Flink**: job parameter `ailake.fetch.columns` (CSV) na `CREATE TABLE` já search-shaped, com o schema declarado na DDL validando contra as colunas pedidas em resolution-time (mesmo padrão de `validateSearchResultSchema`).
  - Documentar em `JVM_INTEGRATION.md` §4D a alternativa de query única (sem JOIN) assim que disponível.
  - Sem trabalho novo em Rust — `ailake_scan_json` já existe e está provado em produção via `duckdb-ailake`; é wiring puro do lado JVM, mesmo formato do fix de `searchMultimodal`. Esforço estimado: ~2-3 semanas (a parte difícil é schema dinâmico — hoje os três plugins fixam o shape de resultado em nível de tipo).
