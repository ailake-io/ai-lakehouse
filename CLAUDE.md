...
### Fase 17 — REST Catalog wiring (CLI, Python, JNI) + 2 bugs reais achados/corrigidos
...
- [x] **2 bugs reais achados e corrigidos via teste live** (container real `apache/iceberg-rest-fixture:latest`, não mock):
  ...
  - Terceiro achado, **não fechado**: `AddPartitionSpec` redundante (reenviava spec inalterado em toda tabela sem particionamento real) corrigido (só emite quando o spec realmente mudou) — mas o path de schema-patch (dispara em TODO commit normal, não só evolução de schema real — ver `TableWriter::commit`'s `captured_schema`) ainda bate num problema não resolvido, aparentemente específico do fixture: `SetCurrentSchema` rejeitado com DOIS erros diferentes pra DUAS formas de request igualmente válidas pelo spec (sentinela `-1` vs id explícito) — não confirmado se é bug do fixture (provável, é implementação de referência minimalista) ou algo mais profundo. Documentado em `docs/guides/REST_CATALOG.md`, não escondido.
- [x] **`docs/guides/REST_CATALOG.md`** — design, auth, uso CLI/Python/JNI, limitações conhecidas (alinhamento manual `--store`/`--rest-warehouse`, schema-patch em aberto, `AddPartitionSpec` particionado não testado), os 2 bugs achados+corrigidos.
- Verificado com build real (`cargo build --workspace`, com e sem `catalog-rest`/`rest-catalog`), suite completa (0 regressões), `cargo clippy` limpo (com e sem a feature), `cargo fmt --all --check` limpo, 2 testes reais `#[ignore]`-por-padrão em `ailake-catalog/src/rest.rs` rodados contra servidor live (`live_create_table_auto_creates_namespace`, `live_ensure_namespace_is_idempotent`), round-trip completo insert+commit+search testado manualmente via `ailake-py` contra o mesmo servidor (create+namespace+auth confirmados funcionando; write-commit bloqueado pelo item 3 acima, documentado como aberto).
